...
class EMPPromarySales(models.TransientModel):
    _name = 'emp.dist.primary.wiz'
    _description = 'Distributors Primary Excel Report Wizard'
    ...
    def generate_excel_report(self):
        ...
        worksheet.merge_range('A1:O1', 'Distributors Primary Sales Report', head_main)
        worksheet.merge_range('A2:O2', title, head)
        ...
        worksheet.write(2, 0, 'Employee', head2)
        worksheet.write(2, 1, 'Distributor', head2)
        worksheet.write(2, 2, 'Product Category', head2)
        worksheet.write(2, 3, 'Product', head2)
        worksheet.write(2, 4, 'Weight (packing)', head_right)
        worksheet.write(2, 5, 'MRP', head_right)
        worksheet.write(2, 6, 'Quantity (KG)', head_right)
        worksheet.write(2, 7, 'Value', head_right)
        worksheet.write(2, 8, 'LM', head_right)
        worksheet.write(2, 9, 'LYSM', head_right)
        worksheet.write(2, 10, 'Growth Over LM (+/-)', head_right)
        worksheet.write(2, 11, 'Growth Over LYSM (+/-)', head_right)
        worksheet.write(2, 12 , 'YTD', head_right)
        ...
        for emp in sales_person_ids:
            worksheet.write(new_row1, 0, emp.name, centered_format)
            ...
            customer_ids = emp_so_ids.mapped('partner_id')
            for customer in customer_ids:
                customer_name = customer.name
                if customer.cust_contact_type == 'distributors':
                    customer_name = customer.name + ' (WD)'
                if customer.cust_contact_type == 'super_stockist':
                    customer_name = customer.name + ' (SS)'
                if customer.cust_contact_type == 'HORECA':
                    customer_name = customer.name + ' (HORECA)'
                worksheet.write(new_row1, 1, customer_name, centered_format)

                cus_so_ids = emp_so_ids.filtered(lambda l: l.partner_id.id == customer.id)
                order_lines = cus_so_ids.mapped('order_line')
                products = order_lines.mapped('product_id')
                
                cus_lm_emp_so_ids = lm_emp_so_ids.filtered(lambda l: l.partner_id.id == customer.id)
                lm_order_lines = cus_lm_emp_so_ids.mapped('order_line')
                cus_lysm_emp_so_ids = lysm_emp_so_ids.filtered(lambda l: l.partner_id.id == customer.id)
                lysm_order_lines = cus_lysm_emp_so_ids.mapped('order_line')
                ...
                for product in products:
                    ...
                    worksheet.write(new_row1, 2, product.categ_id.display_name, centered_format)
                    worksheet.write(new_row1, 3, product.display_name, centered_format)
                    worksheet.write(new_row1, 4, product.weight, right_format)
                    worksheet.write(new_row1, 5, product.mrp, right_format)
                    worksheet.write(new_row1, 6, sum(total_line.mapped('product_uom_qty')), right_format)
                    worksheet.write(new_row1, 7, round(sum(total_line.mapped('price_total'))), right_format)

                    worksheet.write(new_row1, 8, round(sum(lm_total_line.mapped('price_total'))), right_format)
                    worksheet.write(new_row1, 9, round(sum(lysm_total_line.mapped('price_total'))), right_format)
                    
                    worksheet.write(new_row1, 10, round(sum(total_line.mapped('price_total')) - sum(lm_total_line.mapped('price_total'))), right_format)
                    worksheet.write(new_row1, 11, round(sum(total_line.mapped('price_total')) - sum(lysm_total_line.mapped('price_total'))), right_format)
                    # worksheet.write(new_row1, 12, round(sum(total_line.mapped('price_total')) - sum(lysm_total_line.mapped('price_total'))), right_format)
                    new_row1 += 1
        ...
        worksheet.write(new_row1, 6, round(total_qty), right_format_head)
        worksheet.write(new_row1, 7, round(total_value), right_format_head)
        worksheet.write(new_row1, 8, round(total_lm), right_format_head)
        worksheet.write(new_row1, 9, round(total_lysm), right_format_head)
        worksheet.write(new_row1, 10, round(total_gr_lm), right_format_head)
        worksheet.write(new_row1, 11, round(total_gr_lysm), right_format_head)
        ...
        self.write({
            ...
            'report_name': 'Distributors Primary Sales Report.xlsx'
        })
